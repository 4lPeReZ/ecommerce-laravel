                    ...
                    <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <label class="flex items-center">
                                ...
                                <span class="ml-2 text-sm text-principal">{{ $permission }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-footerprincipal">
                                            {{ __('Last used') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif

                                    @if (Laravel\Jetstream\Jetstream::hasPermissions())
                                        <button class="cursor-pointer ml-6 text-sm text-footerprincipal underline" wire:click="manageApiTokenPermissions({{ $token->id }})">
                                            {{ __('Permissions') }}
                                        </button>
                                    @endif
    ...
    <x-jet-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            ...
            <x-jet-input x-ref="plaintextToken" type="text" readonly :value="$plainTextToken"
                class="mt-4 bg-gray-100 px-4 py-2 rounded font-mono text-sm text-principal w-full"
